...
const Log: React.FC<ISettingsProps> = ({ unit, setUnit }) => {

  const [settings, setSettings] = useState<ISettingsState>({name: "", moistLimit: 0, waterTime: 0, moistMeasureIntervall: 0 })
  const modal = useRef<HTMLIonModalElement>(null);

  useEffect(() => {
    setSettings({ name: unit.name, moistLimit: unit.moistLimit, waterTime: unit.waterTime, moistMeasureIntervall: unit.moistMeasureIntervall })

  },[])

  const confirm = () => {
    setUnit({ ...unit, name: settings.name, moistLimit: settings.moistLimit, waterTime: settings.waterTime, moistMeasureIntervall: settings.moistMeasureIntervall })
